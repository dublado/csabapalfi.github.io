...
A working consensus algorithm is typically used to **maintain a replicated log** of commands across multiple nodes. This log then can be used to store command which can control the state of a state machine (be it a key-value store or anything).
...
The point of **feature flags** is to separate deploys from releases. Feature flag ```if``` statements are about sacrificing some local code clarity for global stability.
...
The [Guardian](http://theguardian.com) is doing some pretty cool stuff and certainly is visible on a lot of conferences. Simon talked about how they do continuous delivery with around 100 deployables on a site seeing 100 million monthly unique browsers.

So **how do you know** you're doing continuous delivery? :)

1. Keep your software deployable trough lifecycle
2. Keep it deployable over new features
3. Get fast and automated feedback on production readiness
4. Push button deployment for any deployable to any environment on demand

**Deployment as service** can also be added here to allow setting up new deployables and pipelines in a self-service way. Having an **audit trail** is also key to keep deployments and changes visible. **Autoscaling deploys** is one of the great capabilities enabled by the cloud. Essentially you bring up a new cluster of services for deployment and as soon as the new release is healthy you point your loadbalancer at the new version and kill the old cluster. **Expiration for feature flags** is about not letting your code quality damaged by all the conditional feature flag logic. Great idea.

Simon suggests to **start small** and **encourage envangelism** in your teams. Also important to make everything as easy as possible for developers when it comes to deployment as a service. It can mean a lot if **business priorities** are also followed instead of trying to totally de-risk your approach. The business will be really happy if a critical **bug fix can be rolled out quickly** because of continuous delivery. When it comes to deploying continuously it works really well for APIs but it's a bit controversial whether want it for UI components.
...
**[The E-myth revisited](http://www.amazon.co.uk/myth-Revisited-Small-Businesses-About/dp/0887307280)** is a good book with a bad title. Actually E stands for  entrepreneurship. As developers we have to understand and **work on the business**. Can you make a better burger than McDonalds? Can you **sell** it better?

When it comes to outsourcing lot of people has mixed feelings but **automation** and **outsourcing** might actually have a lot in common. **Make the fuzzy concrete** and **decompose responsibilities** then **automate or outsource**. Chad also mentioned the book titled [4 hour workweek](http://fourhourworkweek.com/) and how you can actually outsource some unusual things in your life like booking appointments or even finding a nice pair of shoes based on a photo. I also learned what Amazon Mechanical Turk was named [after](http://en.wikipedia.org/wiki/The_Turk).


## Graph Search with Neo4j
### by Ian Robinson [@iansrobinson](http://twitter.com/iansrobinson)

I was a bit concerned it turns out to be a marketing thing. Still could be worth [checking out the recording](http://www.ustream.tv/recorded/46666141) because of all the twitter love. It's worth to be aware of that [Neo4j comes with dual licensing](http://www.neo4j.org/learn/licensing): GPL for open source projects and a **paid licence** for anything else.
